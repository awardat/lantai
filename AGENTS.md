...
- ✅ 已执行：0.1.4（用户确认后实施）：供应商新增 OpenCode Go（本地代理，http://127.0.0.1:8787/v1）、DeepSeek 默认模型改为 deepseek-v4-flash、"测试"功能（卡片内展示模型清单可点击填入）、后端日志文件输出（data/logs/，每次启动新文件，保留 20 个）；`release/lantai-0.1.4-windows-x64/` 验证可运行。0.1.4 修订（并入，不另升版）：配置窗口尺寸修正（进入配置后大窗口固定、密码门禁小窗口自适应）、新建版本记录文件（docs/03-增长迭代/版本记录.md，版本号与修改内容不放入 README）。
