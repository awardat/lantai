...
- ✅ 已执行：0.1.49（用户两项，CH-094）：**删除一致性 + 孤儿清理 + 文档清单分页**——① 切片删除统一 `_purge_chunks`（clear/delete/reparse 唯一出口）+ 启动 `gc_orphan_chunks` 清无主切片（实测 sample 库 20502 孤儿→0）；② `GET /api/docs` 可选 page/page_size(20/50/100)/status → {total,page,items,stats}（无参兼容）；前端翻页栏（默认 20，筛选/轮询/删除联动、越界回退）；E2E 25 文档 20+5 分页 ✅、删除后 chunks/fts=0 ✅。
- ✅ 已执行：0.1.50（用户实测报告，CH-097 + 审核整改 CH-098）：**翻页/筛选被轮询覆盖修复**——轮询 `startPolling` 由旧全量逻辑改**复用 `loadDocs()`**（同走 page/page_size/status），停止判断改用全量 `lastStats`（parsing/queued）；0.1.49 的"换筛选/翻页 1 秒被覆盖回全量"消除（语法/断言通过，浏览器端待部署人工复核）；技术对接 §7.8 R110 口径补"Tesseract 已部分落地（0.1.46）/PaddleOCR 仍评估"（CH-098）。
- ✅ 已执行：0.1.51（用户实测报告后决策改方案，CH-099 终版 + CH-100）：**去除文档列表自动轮询**——删除 startPolling/pollTimer/lastStats 与交互保护 guard，解析状态改切换筛选/翻页时手动刷新；轮询对翻页/筛选/下拉的干扰（"还没选中就刷新"/"1 秒被覆盖回全量"）随轮询根除；新增 `docs/03-增长迭代/CHANGELOG.md`（Keep a Changelog 1.1.0 规范）。
- ✅ 已执行：0.1.52（GBT 21054-2023 用户实测，CH-102）：**OCR 全页失败根因修复**——① 全页失败错误透传真实原因（不再"公式/符号扫描件"误导）；② **逐页混合 OCR**（`_ocr_pdf`/`_ocr_pdf_pages` 位图优先、位图为空/缺位图用该页渲染图兜底）——修复"少量装饰位图占用提取结果、渲染回退被整体跳过"（GBT 21054 仅 1 张 6KB 装饰 TIFF → 原先全页失败；实测 28 页全覆盖 29357 字、封面正文也在）。
- 📋 已登记待处理（CH-095，用户"做个MCP协议"并细化方案；含 UI 管理）：**兰台 MCP Server**——方案文档 `docs/02-方案设计/MCP-Server方案.md`（只读 tools：search_knowledge/ask_lantai/list_documents/get_document；stdio + Streamable HTTP 双传输、HTTP 复用 API token Bearer；mcp/fastmcp SDK；chat 核心抽 service 共用；实施前验证 Python 3.14 兼容）；**UI（设置页「MCP」区块）**：MCP 开关（启停→HTTP 404/403）、「安装到 agent」（写 Claude Desktop 配置/展示可复制 JSON、可逆）、「卸载」（移除 agent 配置段+停用）；落点待实施时与用户确认（应用设置页或壳控制台）。
- 📋 备忘（CH-096，企业应用）：**S3 存储支持**——storage 抽象（Local/S3 同接口）+ minio SDK + 预签名 URL 预览 + 分段上传/桶生命周期；演示默认 Local 不变，设置 S3 即切换。
