...
- **变更实施过程中必要的文本必须保存到文档**（方案/结论/中间记录写入 docs/ 与变更记录，不留存于会话）；实施完成后先交付说明（变更登记、版本同步、自测结论），**不自动构建发布**。
- **构建/发布必须等待用户明确确认**（CH-049）：变更完成后先登记 CH、同步版本号与文档、完成自测并汇报；用户确认"发布"后再执行 `scripts/build_release.ps1`（构建 release 目录与 zip），再 git 提交推送。
