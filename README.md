...
> 版本 **0.1.52** ｜ 平台 **Windows x64**（MVP）｜ 单机部署 ｜ 无构建步骤，两步启动
...
| 模块 | 功能 |
|------|------|
| 文档管理 | 上传（≤20MB）、解析状态（**文件计数 + 状态筛选联动**）、文档列表（**翻页：每页 20/50/100，默认 20**）、**失败原因悬停 + 失败重试（可指定文件大类兜底）**、**已就绪文档「重新解析」**（版本升级后按当前方法重造切片，或 `scripts/reparse_all.py` 一键全库重解析）、删除（连同向量与源文件，**启动自动清理孤儿切片**）；**Office/PDF 表格自动转自然语言分块**（数字/指标类检索更准） |
...
验证：`tesseract --list-langs` 应显示 `chi_sim`。未管理员权限时可用免提权方案（把 chi_sim.traineddata 放入 `%LOCALAPPDATA%\Tesseract-OCR\tessdata` 并设置用户环境变量 `TESSDATA_PREFIX` 指向该目录）。中文为简体；需繁体再加 `chi_tra`。启用本地 OCR 但 Tesseract/语言数据缺失时，解析失败提示中会附安装指引。**OCR 结果自动清洗去噪、同文档切片去重**（公式/水印类扫描件的噪声伪文本不入库，0.1.47）；**本地 OCR 识别文字与耗时记录于 `data/logs/agent-*.log`（slot=`ocr_local`）**，与 AI 调用日志同文件可查（0.1.48）。
