...
> 版本 **0.1.20** ｜ 平台 **Windows x64**（MVP）｜ 单机部署 ｜ 无构建步骤，两步启动
...
解压 `release/lantai-0.1.20-windows-x64.zip`，运行目录内 `lantai.exe`，浏览器打开 http://127.0.0.1:8000。

### 方式三：桌面壳（单机应用形态）

解压 `release/lantai-shell-0.1.20-windows-x64.zip`，双击目录内 `lantai-shell.exe`——壳自动在内部拉起兰台服务（**无独立 cmd 窗口**），就绪后直接显示兰台界面；右下角「终端」按钮可查看服务日志/重启/停止；关闭窗口即退出并结束服务。数据仍保存在壳目录的 `data/` 下。
