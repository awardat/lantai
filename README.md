...
> 版本 **0.1.37** ｜ 平台 **Windows x64**（MVP）｜ 单机部署 ｜ 无构建步骤，两步启动
...
| 模块 | 功能 |
|------|------|
| 文档管理 | 上传（≤20MB）、解析状态（**文件计数 + 状态筛选联动**：全部/已就绪/排队中/解析中/失败）、文档列表、**失败原因悬停 + 失败重试（可指定文件大类兜底：文本/Office/文字 PDF/图片 PDF·OCR/图片）**、删除（连同向量与源文件） |
