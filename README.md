...
| 模块 | 功能 |
|------|------|
| 文档管理 | 上传（≤20MB）、解析状态（**文件计数 + 状态筛选联动**）、文档列表、**失败原因悬停 + 失败重试（可指定文件大类兜底）**、**已就绪文档「重新解析」**（版本升级后按当前方法重造切片，或 `scripts/reparse_all.py` 一键全库重解析）、删除（连同向量与源文件）；**Office/PDF 表格自动转自然语言分块**（数字/指标类检索更准） |
