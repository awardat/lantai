...
> 版本 **0.1.25** ｜ 平台 **Windows x64**（MVP）｜ 单机部署 ｜ 无构建步骤，两步启动
...
> **监听地址**：服务默认仅监听本机回环地址 `127.0.0.1:8000`（不对外网开放、不接受局域网访问）。如需远程调用/局域网访问测试，请自行修改 `backend/app/config.py` 的 `DEFAULT_HOST`（如 `0.0.0.0`）并重新打包——演示产品默认不开放远程（安全考虑）。
